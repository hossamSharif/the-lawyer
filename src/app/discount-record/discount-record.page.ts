...
@Component({
  ...
  templateUrl: './discount-record.page.html',
  styleUrls: ['./discount-record.page.scss'],
})
export class DiscountRecordPage implements OnInit {
  payArray:Array<any> =[]
  printArr:Array<any> =[]
...
//
  ...
  search(){
   ...
   getTopSales(){  
    this.loading = true
    this.api.getTopDiscount(this.store_info.id, this.year.id).subscribe(data =>{
       //console.log('hhhhhh',data)
       let res = data
       if(res['message'] != 'No record Found'){
         this.payArray = res['data'] 
       } else{
         this.presentToast('لا يوجد بيانات' , 'danger')
       }
     }, (err) => {
      this.presentToast('خطا في الإتصال حاول مرة اخري' , 'danger')
